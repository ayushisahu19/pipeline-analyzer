...
│   │
│   ├── models/
│   │   └── PipelineRun.js    # Schema (branch, buildTime, status, vulnerabilities)
│   │
│   ├── controllers/
...
Run Tests → Executes npm test -- --passWithNoTests

Vulnerability Scan → Runs npm audit
...
Jenkins → Pipeline Execution → Metrics Collection → Backend API → MongoDB → Frontend Dashboard
...
Custom pipeline metrics stored in MongoDB
